...
class AutoencoderValidationCallback(keras.callbacks.Callback):
    """Class to define a callback to compute validation loss for autoencoder during training.
    """
    def __init__(self, validation_data, validation_loss_fn, metric_name='val_autoencoder_loss'):
        """Initialize the callback.

        Args:
            validation_data (list, np.ndarray): Validation data.
            validation_loss_fn (Callable): Validation loss function.
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        val_loss = self.validation_loss_fn(X_val)
...
def _create_autoencoder_loss_fn(model_instance):
    ...
    def _val_autoencoder_loss_fn(X_val):
        ...
        if loss_fn is None:
            raise ValueError("No reconstruction loss function defined in the model instance.")
        ...
        if isinstance(autoencoder_loss, tf.Tensor):
            ...
            raise TypeError(f"Unexpected type for classification_loss: {type(autoencoder_loss)}")
...
class TrackedPUMAP(parametric_umap.ParametricUMAP):
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state, **kwargs):
        ...
        if self.parametric_reconstruction and self.autoencoder_loss:
            if self.X_val is not None:
                _val_classification_loss_fn = create_autoencoder_loss_fn(self)
                autoencoder_val_callback = AutoencoderValidationCallback(
